      ...
      {isOpen ? (
        ...
        >
          ...
          >
            ...
            {status === "form" ? (
              <>
                <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-gold">Assinar CTV Imperial</p>
                <h2 id={titleId} className="mt-2 font-display text-3xl text-ivory">
                  Quase no seu sofá.
                </h2>
                <p className="mt-2 text-sm text-mist">Escolha o plano, preencha seus dados e ative em minutos.</p>
                ...
                <div className="mt-6 grid grid-cols-3 gap-2">
                  {plans.map((item) => (
                    ...
                    >
                      <span className="block text-[11px] text-mist">{item.name}</span>
                      <span className="mt-1 block text-sm font-semibold text-ivory">{formatBRL(item.price)}</span>
                    </button>
                  ))}
                </div>
                ...
                <form className="mt-6 space-y-3" onSubmit={onSubmit}>
                  ...
                  <Field label="WhatsApp" name="phone" type="tel" autoComplete="tel" placeholder="(11) 99999-0000" required />

                  <fieldset className="pt-2">
                    <legend className="mb-2 text-xs font-medium text-mist">Pagamento</legend>
                    <div className="grid grid-cols-2 gap-2">
                      <PaymentOption
                        active={payment === "pix"}
                        onClick={() => setPayment("pix")}
                        title="PIX"
                        hint="Liberação imediata"
                      />
                      <PaymentOption
                        active={payment === "card"}
                        onClick={() => setPayment("card")}
                        title="Cartão"
                        hint="Crédito em até 1x"
                      />
                    </div>
                  </fieldset>
              ...
              <div className="py-6 text-center">
                ...
                <h2 id={titleId} className="mt-5 font-display text-3xl text-ivory">
                  Bem-vindo ao império.
                </h2>
                <p className="mx-auto mt-3 max-w-sm text-sm leading-relaxed text-mist">
                  Recebemos seu pedido do plano <strong className="text-ivory">{plan.name}</strong>. Em instantes o
                  acesso chega no e-mail e no WhatsApp para você começar a assistir.
                </p>
                <button
                  type="button"
                  onClick={closeCheckout}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? "border-gold bg-gold/10" : "border-white/10 hover:border-gold/30",
      )}
    >
